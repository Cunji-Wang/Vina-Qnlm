...
void model::assign_types() {
	VINA_FOR(i, grid_atoms.size() + atoms.size()) {
		...
		switch(a.el) {
			...
			case EL_TYPE_C    : x = (a.ad == AD_TYPE_A) ? XS_TYPE_C_A : ((bonded_to_heteroatom(a)&& a.ad == AD_TYPE_C)? XS_TYPE_C_P:XS_TYPE_C_H );break;
				//x = bonded_to_heteroatom(a) ? XS_TYPE_C_P : XS_TYPE_C_H; break;
			...
			case EL_TYPE_S    : x = (a.ad == AD_TYPE_SA)? XS_TYPE_S_H : XS_TYPE_S_P; break;
...
void model::write_context(const context& c, ofile& out) const {
	...
	VINA_FOR_IN(i, c) {
		...
		if(c[i].second) {
			result = result +coords_to_pdbqt_string(coords[c[i].second.get()], str)+'\n';
...
void model::write_context_new(const context& c) const {
        verify_bond_lengths();
	//if (verify_global==1){
	//	result = result + "bond excrption!\n";
	//	verify_global = 0;
	//}else{
        VINA_FOR_IN(i, c) {
                const std::string& str = c[i].first;
...
std::string model::ligand_atom_str(sz i, sz lig) const
{
	...
	for(sz c = 0, nc = cont.size(); c < nc; c++)
	{
		...
		{
			pdbline = cont[c].first;
			break;
		}
	}

	if(pdbline.size() > 6)
		return pdbline.substr(6,10);
	else
		return "";
}
...
#ifdef ATHREAD_MC
void model::initializepara(const precalculate& prec){
	
	
		
	//alloc the matrix size, this 3 dimenssion matrix will be used for derivate evaluation
	if(smoothInit != true){
        	sz n = prec.element_size();
        	sz matsize = prec.matrix_size() * (prec.matrix_size()+1)/2;
		//printf("smooth ======================%d,,,,,,%d\n",n ,matsize);
/*        	mysmooth = (fl ***)malloc(sizeof(double**)*matsize);
        	for(int i=0; i<matsize;i++)
                	mysmooth[i] = (fl **)malloc(sizeof(double*)*n);
        	for(int i=0; i<matsize;i++)
                	for(int j =0; j<n; j++)
                        	mysmooth[i][j] = (fl *)malloc(sizeof(double)*2);
*/

        	int totalsize = matsize*n*2;
//        	printf("mysmooth matsize is %d, n is %d, total is %d\n", matsize, n, totalsize);        
        //	prec.init_smooth();
		smoothInit = true;
		...
		smooth_slave = (fl *)malloc(sizeof(double) * matsize * n * 2);
        	prec.init_smooth();

/*	        for(int i=1; i<slave_num; i++){
                        memcpy(&smooth_slave[totalsize*i], &smooth_slave[0], sizeof(double) * totalsize);
                }
*/	
...
void model::freepara(const precalculate& prec){
    sz n = prec.element_size();
    sz matsize = prec.matrix_size() * (prec.matrix_size()+1)/2;
...
/*
        	for(int i=0; i<matsize;i++)
                	for(int j =0; j<n; j++)
                        	free((void *)mysmooth[i][j]);
        	for(int i=0; i<matsize;i++)
                	free((void *)mysmooth[i]);
        	free((void *)mysmooth);
*/
		//lu:
        	free((void *)smooth_slave);
		smoothInit = false;
	}
}
#endif
	...
	VINA_FOR(i, grid_atoms.size() + atoms.size()) {
		...
		VINA_FOR_IN(j, a.bonds) {
			...
			if(!ok) {
				...
				//temp modify,stop verify exception
				//verify_global = 1;
			}
