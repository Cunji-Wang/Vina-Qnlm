...
struct parallel_mc_task {
	...
	rng generator;
	#ifdef ATHREAD_MC
	parallel_mc_task(const model& m_, tranModel *tm_, int seed) : m(m_), tm(tm_), generator(static_cast<rng::result_type>(seed)) {}
	#else
...
struct parallel_mc_aux {
	...
	#ifdef ATHREAD_MC
	(*mc)(t.m, t.tm, t.out, *p, *ig, *p_widened, *ig_widened, *corner1, *corner2, pg, t.generator);
	#else
...
#ifdef ATHREAD_MC
void parallel_mc::operator()(const model& m, output_container& out, const precalculate& p, const igrid& ig, const precalculate& p_widened, const igrid& ig_widened, const vec& corner1, const vec& corner2, rng& generator, tranModel *tm) const {
#else
...
//lu modify
	...
	task_container.push_back(new parallel_mc_task(m, tm, random_int(0, 1000000, generator)));
	parallel_mc_aux_instance(task_container[0]);
/*
