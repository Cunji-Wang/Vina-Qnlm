...
struct precalculate {
	...
	#ifdef ATHREAD_MC
	fl element_size()const { return n; }
    fl matrix_size() const {return data.dim();}
	#endif
	...
	#ifdef ATHREAD_MC
	void init_smooth(void) const {
		sz dim = data.dim();
		sz size = dim*(dim+1)/2;
        	VINA_FOR(t1, size){
            		const precalculate_element& p = data(t1);
			VINA_FOR_IN(i, p.smooth){
				smooth_slave[t1*p.smooth.size()*2 + i*2 + 0] = p.smooth[i].first;
				smooth_slave[t1*p.smooth.size()*2 + i*2 + 1] = p.smooth[i].second;
                	//	mysmooth[t1][i][0] = p.smooth[i].first;
                	//	mysmooth[t1][i][1] = p.smooth[i].second;
             	/*		printf("t1 is %d, i is %d, smooth size is %ld, data size is %d\n", 
					t1, i, p.smooth.size(),size);
                        	printf("first is %lf, second is %lf\n", mysmooth[t1][i][0], mysmooth[t1][i][1]);
		*/
              		}	
		}
    }
