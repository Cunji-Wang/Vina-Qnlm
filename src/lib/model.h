...
struct model {
	...
	void write_structure(ofile& out) const {
        ...
        void write_structure_new() const {
                VINA_FOR_IN(i, ligands)
                        write_context_new(ligands[i].cont);
                if(num_flex() > 0) // otherwise remark is written in vain
                        write_context_new(flex_context);
        }
	...
	void write_structure_new(const std::string& remark) const {
                //out << remark;
                result = result + remark;
                write_structure_new();
        }
	...
	void write_model(ofile& out, sz model_number, const std::string& remark) const {
		...
		result = result +"MODEL "+std::to_string(model_number)+'\n';
		write_structure(out, remark);
		//out << "ENDMDL\n";
		result = result +"ENDMDL\n";
	}
	void write_model_new(sz model_number, const std::string& remark) const {
		//out << "MODEL " << model_number << '\n';
		result = result +"MODEL "+std::to_string(model_number)+'\n';
		write_structure_new(remark);
		//out << "ENDMDL\n";
		result = result +"ENDMDL\n";
        } 
	...
	void about() const;
	#ifdef ATHREAD_MC
	void initializepara(const precalculate& prec);
	void freepara(const precalculate& prec);
	#endif
	...
	#ifdef ATHREAD_MC
    atomv get_grid_atoms() const {
        return grid_atoms;
    }
    atomv get_ligand_atoms() const {
        return atoms;
    }

    interacting_pairs get_ligand_pairs() const {
        interacting_pairs tmp;
        VINA_FOR_IN(i, ligands) {
    ...
    struct rigid_body get_rootbody() const{
        VINA_FOR_IN(i, ligands) {
            const ligand& lig = ligands[i];
            struct rigid_body tmp = lig.node;
            return tmp;
        }
    }
...
private:
	...
	void write_context_new(const context& c) const;
