...
private:
	void loop() {
		#ifdef ATHREAD_MC
		printf("athread is inited in the loop\n");
		athread_init();
		#endif
		while(boost::optional<sz> i = get_next()) {
			printf("loop i is %d\n", finished);
		// it will call parallel_iter.aux(i) then callthe parallel_mc_aux operator which will call the monte-carlo function
			(*m_f)(i.get()); 
			{
				boost::mutex::scoped_lock self_lk(self);
				++finished;
	...
	//	printf("athread is halted after the loop is finshed\n");
	//	athread_halt();
	}
    struct aux {
        parallel_for* par;
...
private:
	struct aux {
		const F* f; //parallel_mc_aux
		Container* v;//parallel_mc_task_container  
