...
struct monte_carlo {
...
#ifdef ATHREAD_MC
	output_type operator()(model& m, tranModel *tm, const precalculate& p, const igrid& ig, const precalculate& p_widened, const igrid& ig_widened, const vec& corner1, const vec& corner2, incrementable* increment_me, rng& generator) const;
#else
	...
	// out is sorted
	#ifdef ATHREAD_MC
	void operator()(model& m, tranModel *tm, output_container& out, const precalculate& p, const igrid& ig, const precalculate& p_widened, const igrid& ig_widened, const vec& corner1, const vec& corner2, incrementable* increment_me, rng& generator) const;
	#else
